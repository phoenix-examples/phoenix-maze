defmodule HelloPhoenix.GameAgent do
  ...
  def revive_player(playerId) do
    Agent.get_and_update(:rawkets, fn(s) -> {:ok, %{s | players: HashDict.update!(s.players, playerId, fn(s) -> %{s | alive: true } end)}} end)
  end

  def remove_player(playerId) do
    Agent.get_and_update(:rawkets, fn(s) -> {:ok, %{s | players: HashDict.drop(s.players, [playerId])}} end)
  end
